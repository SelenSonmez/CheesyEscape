...
public class PlayerScript : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (other.gameObject.CompareTag("Obstacle"))
        {
            ...
            if (!isChased)
            {
                ...
                StartCoroutine(wait());
                StartCoroutine(animationWait(other.gameObject));
            }
            else
            {
                if (animatedObjects.Contains(other.gameObject)) return;
    ...
    IEnumerator animationWait(GameObject obstacle)

    {
        animatedObjects.Add(obstacle);
        yield return new WaitForSeconds(0.4f);
        animatedObjects.Remove(obstacle);
    }
